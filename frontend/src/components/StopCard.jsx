  ...
  return (
    ...
    >
      ...
      {!isOverlay && (
        <>
          <div className="absolute left-0 sm:left-24 top-0 bottom-0 w-px bg-gray-200 dark:bg-gray-800"></div>
          <div className="absolute left-[-4px] sm:left-[92px] top-8 w-3 h-3 rounded-full bg-blue-500 ring-4 ring-white dark:ring-gray-900 transition-transform group-hover:scale-125 duration-200"></div>
        </>
      )}
      ...
      {/* Card Content */}
      <div className={`bg-white dark:bg-gray-900 rounded-xl border p-5 transition-all duration-200 relative ${
          ...
          'border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md group-hover:border-blue-100 dark:group-hover:border-blue-900/50'
        }`}
      >
        ...
        <div className={`absolute -top-3 right-4 bg-white dark:bg-gray-900 shadow-sm border rounded-lg flex items-center overflow-hidden transition-opacity duration-200 ${
            isOverlay || isDragging ? 'opacity-100 border-blue-200 dark:border-blue-800' : 'opacity-0 group-hover:opacity-100 border-gray-100 dark:border-gray-800'
          }`}
        >
          {/* Drag Handle */}
          <button 
            {...attributes} 
            {...listeners}
            className={`p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 dark:hover:text-gray-300 ${isOverlay ? 'cursor-grabbing' : 'cursor-grab'}`}
            title="Drag to move"
          ...
          <button 
            ...
            className="p-1.5 hover:bg-yellow-50 dark:hover:bg-yellow-900/20 border-l border-gray-100 dark:border-gray-800 transition-colors"
            title={stop.isFavorite ? "Remove favorite" : "Mark as favorite"}
            aria-label="Toggle favorite"
          >
            <svg className={`w-4 h-4 transition-colors ${stop.isFavorite ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300 dark:text-gray-500 hover:text-yellow-400'}`} viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
            </svg>
          </button>

          <button onClick={() => setIsEditing(true)} className="p-1.5 hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-600 dark:text-blue-400 border-l border-gray-100 dark:border-gray-800" title="Edit" aria-label="Edit stop">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
          </button>
          
          <button onClick={onDelete} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 dark:text-red-400 border-l border-gray-100 dark:border-gray-800" title="Delete" aria-label="Delete stop">
        ...
        >
          <div className="mt-2 text-gray-600 dark:text-gray-400 text-sm leading-relaxed border-t border-gray-50 dark:border-gray-800 pt-3">
            ...
            <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 font-medium text-xs bg-gray-50 dark:bg-gray-800/50 inline-block px-3 py-1.5 rounded-lg">
